...
RUN curl -sfo "${_FLUTTER_TAR}" "${_FLUTTER_STORAGE}/${_FLUTTER_TAR}" \
    && mkdir /flutter \
    && tar -xf "${_FLUTTER_TAR}" -C /flutter --strip-components=1

FROM eclipse-temurin:${_JAVA_VERSION}-jdk AS jdk

FROM curl AS android-sdk
...
ARG _FLUTTER_SUPPRESS_ANALYTICS="true"
ARG _ANDROID_SDK_ROOT="/opt/android-sdk"
...
ENV FLUTTER_SUPPRESS_ANALYTICS="${_FLUTTER_SUPPRESS_ANALYTICS}"
...
RUN export _DISTRO="${_DISTRO_NAME}" \
    ...
    && pkg_clean \
    && git config --system --add safe.directory "${_FLUTTER_HOME}" \
    ...
    && chown -R "${_USERNAME}:${_USERNAME}" \
        "${_HOME}" "${_PUB_CACHE}" "${_GRADLE_HOME}" \
        "${_ANDROID_HOME}" "${_FLUTTER_HOME}" "${_ANDROID_SDK_ROOT}"
